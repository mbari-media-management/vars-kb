...
public class RejectHistoryTask extends AbstractHistoryTask {
            ...
            if (thisConcept.equals(thatConcept)) {
                ...
                if (thisConcept.getPrimaryConceptName().getName().equals(name)) {
                    ...
                    Thread thread = new Thread(() -> {
                        try {
                            final String newName = thisConcept.getPrimaryConceptName().getName();
                            final Collection<String> oldNames = thisConcept.getConceptNames()
                                    .stream()
    ...
    private class AddConceptTask extends GenericRejectTask {
        ...
        public void reject(final UserAccount userAccount, History history, DAO dao) {
            ...
            List<String> namesToChange = conceptCache.findDescendantNamesFor(rejectedConcept);
            int n = annotationService.countByConcepts(namesToChange);

            final String newName = parentConcept.getPrimaryConceptName().getName();
            final String msg = n + " Observations were found using '" + rejectedName +
    ...
    private class AddMediaTask extends GenericRejectTask {
        ...
        public void reject(UserAccount userAccount, History history, DAO dao) {
            if (canDo(userAccount, history)) {
                ...
                final Collection<Media> matches = mediaSet.stream()
                        .filter(input -> input.getUrl().equals(aHistory.getNewValue()))
                        .collect(Collectors.toList());
            ...
            if (canDo(userAccount, history)) {
                ...
                if (currentParent.equals(newParent)) {
                    if (oldParent != null) {
                        ...
                        Collection<Concept> matches = descendents.stream()
                                .filter(input -> input.getPrimaryConceptName().getName().equals(
